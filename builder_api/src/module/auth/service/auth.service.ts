...
import {CreateDeviceUuidDto} from "../dto/createDeviceUuid.dto";
import {DeviceUuidEntity} from "../entity/deviceUuid.entity";
import {auths, devices, otps} from "../../../../drizzle/schema/auth.schema";
...
export class AuthService {
    ...
    async createAuth(args: { deviceUuid: string, userId: string }) {
        await this.db.select().from(auths).where(eq(auths.userId, args.userId)).get();
        const auth = await this.db.insert(auths).values({
            id: uuidv4(),
